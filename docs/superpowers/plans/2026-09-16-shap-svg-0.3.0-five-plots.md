...
export function ShapScatter({
  ...
  return (
    <svg width={width} height={height} role="img"
      ...
      {hovered !== null && (
        <title>
          {`${formatShapValue(parsed.values[hovered][0])}`}
        </title>
      )}
    </svg>
  );
  ...
  return (
    <svg width={width} height={layout.height} role="img"
      ...
      <text x={layout.baseValueX} y={layout.plotBottom + 18}
        ...
        {`${words.baseValue} ${formatLevel(layout.xDomain[0] / 2 + layout.xDomain[1] / 2)}`}
      </text>
...
Append to `src/core/barLayout.ts`:
...
export function clusteredOrder(input: ClusteredOrderInput): {
  ...
  const importanceWithinPool = pool.map((j, position) => position);
...
```python
def write_new_plot_goldens(explanation, name, out_dir):
    """Golden values for the charts added in shap-svg 0.3.0."""
    import numpy as np
    import sklearn.decomposition

    values = np.asarray(explanation.values, dtype=float)
    base = float(np.asarray(explanation.base_values, dtype=float).ravel()[0])
    max_display = 15

    # Decision: SHAP orders by ascending sum|phi| and draws the last rows.
    feature_idx = np.argsort(np.sum(np.abs(values), axis=0))
    shown = feature_idx[-max_display:]
    hidden = feature_idx[:-max_display]
    ordered = values[:, shown]
    starts = base + values[:, hidden].sum(axis=1)
    cumsum = starts[:, None] + np.cumsum(ordered, axis=1)
    write_json(out_dir / f"{name}.decision.golden.json", {
        "max_display": max_display,
        "base_value": base,
        "row_labels": [explanation.feature_names[j] for j in shown],
        "starts": starts.tolist(),
        "cumsum": cumsum.tolist(),
    })

    # Force: one Sample, every contribution, meeting at f(x).
    sample_index = 0
    write_json(out_dir / f"{name}.force.golden.json", {
        "sample_index": sample_index,
        "base_value": base,
        "fx": float(base + values[sample_index].sum()),
        "contributions": values[sample_index].tolist(),
    })

    # Embedding: sklearn's own PCA, which shap.plots.embedding calls.
    pca = sklearn.decomposition.PCA(2)
    coords = pca.fit_transform(values)
    write_json(out_dir / f"{name}.embedding.golden.json", {
