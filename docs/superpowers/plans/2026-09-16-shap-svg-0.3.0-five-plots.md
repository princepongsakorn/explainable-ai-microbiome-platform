...
Both files compare through `expectGoldenNumber`, copied from `tests/golden.waterfall.test.ts` — the
payload is written at four significant figures, so a golden computed from the raw arrays can never
match at six decimal places, and this repository already duplicates that helper per file rather than
sharing it.
...
describe.each(["tiny", "real"])("decision golden values — %s", (name) => {
  ...
  it("draws the same rows, least important first", () => {
